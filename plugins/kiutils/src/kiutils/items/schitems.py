...
class LocalLabel:
    ...
    def from_sexpr(cls, exp: list) -> LocalLabel:
        ...
        for item in exp[2:]:
            ...
            if item[0] == "uuid":
                object.uuid = item[1]
            if item[0] == "fields_autoplaced":
                object.fieldsAutoplaced = True
        return object
...
class SymbolProjectPath:
    ...
    def to_sexpr(self, indent=4, newline=True) -> str:
        ...
        expression += (
            f'{indents}  (reference "{dequote(self.reference)}") (unit {self.unit})\n'
        )
        expression += f"{indents}){endline}"
        return expression
...
class SchematicSymbol:
    ...
    def from_sexpr(cls, exp: list) -> SchematicSymbol:
        ...
        for item in exp[1:]:
            ...
            if item[0] == "dnp":
                object.dnp = True if item[1] == "yes" else False
            if item[0] == "at":
                object.position = Position().from_sexpr(item)
            if item[0] == "property":
                object.properties.append(Property().from_sexpr(item))
    ...
    def to_sexpr(self, indent=2, newline=True) -> str:
        ...
        lib_name = (
            f' (lib_name "{dequote(self.libName)}")' if self.libName is not None else ""
        )
    ...
    def from_sexpr(cls, exp: list) -> HierarchicalSheet:
        ...
        for item in exp[1:]:
            ...
            if item[0] == "property":
                ...
                if item[1] == "Sheet name" or item[1] == "Sheetname":
                    object.sheetName = p
                elif item[1] == "Sheet file" or item[1] == "Sheetfile":
                    object.fileName = p
                else:
